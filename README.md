...
- **Secure Storage**: Encrypted audio chunks are stored in the browser's IndexedDB. Keys are temporarily managed in memory (React state) for the active session to improve security.
- **Performance Metrics**:
...
3.  **Storage**: Encrypted chunks and their IVs are saved to IndexedDB. The encryption key is temporarily saved to memory to allow immediate playback in the demo.
