...
function App() {
  ...
  const encryptAndStore = async () => {
    ...
    // Store key for decryption in memory (React state)
    // Keys are non-extractable and ephemeral to the session
  ...
  const decryptAndPlay = async () => {
      if (!sessionData) {
          addLog("No active session in memory.");
          return;
      }

      const { key, sessionId, algorithm } = sessionData;
