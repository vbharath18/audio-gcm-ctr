...
- **Secure Storage**: Encrypted audio chunks are stored in the browser's IndexedDB. Keys are temporarily managed in ephemeral memory for the active session.
- **Performance Metrics**:
...
3.  **Storage**: Encrypted chunks and their IVs are saved to IndexedDB. The encryption key is temporarily held in ephemeral application memory to allow immediate playback in the demo.
